...
template <class T> ListElement<T>::ListElement(T itm, int sortKey) {
  item = itm;
  key = sortKey;
...
template <class T> List<T>::List() { first = last = NULL; }
...
template <class T> void List<T>::Mapcar(VoidFunctionPtr func) {
...
template <class T> void List<T>::SortedInsert(T item, int sortKey) {
...
template <class T> T List<T>::SortedRemove(int *keyPtr) {
...
template <class T> T List<T>::SortedPeek(int *keyPtr) {
  ListElement<T> *element = first;
  T thing;
