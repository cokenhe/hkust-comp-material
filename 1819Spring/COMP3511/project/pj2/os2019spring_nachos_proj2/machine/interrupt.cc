...
void Scheduler::InterruptHandler(int dummy) {
  ...
  case SCHED_RR: {
    // YOUR PROJECT2 CODE HERE
    // printf(">>> CURRENT THREAD: %s, BURST TIME: %d, SYSTEM TICK: %d, TOTAL TICK: %d\n\n", 
            // currentThread->getName(), currentThread->getBurstTime(),stats->systemTicks, stats->totalTicks);
    int quantum = currentThread->getQuantum();
    if (quantum == 0) 
      interrupt->YieldOnReturn();
    else 
      currentThread->setQuantum(quantum - 1);
    break;
  }
