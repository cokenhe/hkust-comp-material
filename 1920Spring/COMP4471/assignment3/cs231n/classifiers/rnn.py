...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # step 3
        h, cache_rnn = rnn_forward(x, h0, Wx, Wh, b) if self.cell_type == 'rnn' else lstm_forward(x, h0, Wx, Wh, b)
        # step 4
        score, cache_score = temporal_affine_forward(h, W_vocab, b_vocab)
        # step 5
        loss, dloss = temporal_softmax_loss(score, captions_out, mask)
        # BP
        dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dloss, cache_score)
        dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, cache_rnn) if self.cell_type == 'rnn' else lstm_backward(dh, cache_rnn)
    ...
    def sample(self, features, max_length=30):
        ...
        prev_c = 0
        prev_word = np.array([[self._start] * N]).T  # make it (N, 1)
        for i in range(max_length):
            ...
            prev_word = np.array([captions[:, i]]).T  # make it (N, 1)
